...
let brushes_path = path.resolve('images/brushes/brushes.json');
console.log(brushes_path);
let brushes = jsonfile.readFileSync(brushes_path);
let notepad: any = {};
...
function writeDrawingJSON() {
    let drawing_path = path.resolve('images/drawings/captured-drawing.json');
    console.log(drawingHistory.json);
    jsonfile.writeFileSync(drawing_path, drawingHistory.json);
}
